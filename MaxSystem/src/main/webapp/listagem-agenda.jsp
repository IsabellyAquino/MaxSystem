				...
				<ul class="navbar-nav">
					<li>
						<a href="<%=request.getContextPath()%>/listagem-agendar"
	                       class="nav-link">Listagem de agendamentos</a>
                    </li>
				</ul>
			</nav>
		</header>
		...
		<div class="container">
			...
			<div class="container text-right">
				<a href="<%=request.getContextPath()%>/novo"
	               class="btn btn-outline-success btn-md mb-3" style="border: none;">
	               Adicionar
			...
			<div class="container text-right">
				<a href="<%=request.getContextPath()%>/apartamento"
	               class="btn btn-outline-primary btn-md" style="border: none;">
	               Meus agendamentos
				</a>
			</div>
			...
			<table class="table table-hover table-sm shadow">
				...
				<tbody class="align-middle">
					<c:forEach var="agendar" items="${listarAgenda}">
						<tr>
							<td>
								<a href="editar?id=<c:out value='${agendar.id}' />"
								...
								<a href="deletar?id=<c:out value='${agendar.id}' />"
							...
							<td><c:out value="${agendar.id}" /></td>
							<td><c:out value="${agendar.date}" /></td>
							<td><c:out value="${agendar.time}" /></td>
							<td><c:out value="${agendar.apartamento}" /></td>
						</tr>
					</c:forEach>
				</tbody>
			</table>
